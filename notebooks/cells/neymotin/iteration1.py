...
class SPI6(object):
    ...
    def set_axong(self):
        axon = self.axon
        axon.gbar_neykdmc = gbar_kdmc * kdmc_gbar_axonm
        axon.gbar_neynax = gbar_nax * nax_gbar_axonm
        axon.gbar_neykdr = gbar_kdr * kdr_gbar_axonm
        axon.gbar_neykap = gbar_kap * kap_gbar_axonm

    def set_calprops(self, sec):
        sec.gcalbar_neycal = cal_gcalbar
        sec.gcanbar_neycan = can_gcanbar
        sec.gpeak_neykBK = kBK_gpeak
        sec.caVhmin_neykBK = -46.08 + kBK_caVhminShift
        sec.depth_neycadad = cadad_depth
        sec.taur_neycadad = cadad_taur

    def set_somag(self):
        sec = self.soma
        sec.gbar_neyih = gbar_h  # Ih
        self.set_calprops(sec)
        sec.gbar_neykdmc = gbar_kdmc
    ...
    def set_apicg(self):
        ...
        for sec in self.apic:
            ...
            for seg in sec:
                ...
                if d <= nexusdist:
                    seg.gbar_neyih = gbar_h * exp(d / h_lambda)
                else:
                    seg.gbar_neyih = h_gbar_tuft
        self.apic[1].gcalbar_neycal = cal_gcalbar * calginc  # middle apical dend gets more iL
    ...
    def set_props(self):
        ...
        for sec in self.all_sec:
            ...
            sec.gbar_neynax = gbar_nax  # Na
            sec.gbar_neykdr = gbar_kdr  # KDR
            sec.vhalfn_neykdr = kdr_vhalfn  # KDR kinetics
            sec.gbar_neykap = gbar_kap  # K-A
            sec.vhalfn_neykap = kap_vhalfn  # K-A kinetics
            sec.vhalfl_neykap = kap_vhalfl
            sec.tq_neykap = kap_tq
        self.set_somag()
        self.set_bdendg()
        self.set_apicg()
        self.set_axong()
